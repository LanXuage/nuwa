import pytest
import asyncio
from src.nuwa.tool import ToolsManager, Tool, ToolEntity, ToolObjectParameter, ToolParameter
...
async def test_call_tool_sync():
    ...
    manager.add_tool(tool)
    # 调用工具
    from src.nuwa.chat import Function
    func = Function(name="sync_tool", arguments='{"x": 5}')
...
async def test_call_tool_async():
    ...
    manager.add_tool(tool)
    from src.nuwa.chat import Function
    func = Function(name="async_tool", arguments='{"text": "hello"}')
    result = await manager.call_tool(func)
    assert result == "HELLO"
...
async def test_call_tool_with_list_args():
    ...
    manager.add_tool(tool)
    from src.nuwa.chat import Function
    func = Function(name="list_tool", arguments='[1, 2, 3]')
    result = await manager.call_tool(func)
    assert result == 6
...
async def test_call_tool_not_found():
    ...
    manager = ToolsManager()
    from src.nuwa.chat import Function
    func = Function(name="missing", arguments='{}')
