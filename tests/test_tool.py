...
from src.nuwa.tools.models import (
    Tool,
    ToolEntity,
    ToolInvocation,
    ToolObjectParameter,
...
def test_tool_decorator():
    ...
    @manager.tool(
        ...
        parameters=ToolObjectParameter(
            type="object",
            properties={"param": ToolParameter(type="string", description="参数")},
        ),
    )
...
async def test_call_tool_sync():
    ...
    tool_entity = ToolEntity(
        ...
        parameters=ToolObjectParameter(
            type="object",
            properties={"x": ToolParameter(type="number", description="数字")},
        ),
    )
...
async def test_call_tool_async():
    ...
    tool_entity = ToolEntity(
        ...
        parameters=ToolObjectParameter(
            type="object",
            properties={"text": ToolParameter(type="string", description="文本")},
        ),
    )
...
async def test_call_tool_with_list_args():
    ...
    manager = ToolKit()
    tool_entity = ToolEntity(
        name="list_tool", parameters=ToolParameter(type="array", description="数组")
    )
